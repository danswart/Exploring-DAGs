...
# library(dplyr) # A Grammar of Data Manipulation (using namespace syntax instead)
...
X <- 2 + 0.08 * Z + stats::rnorm(n, mean = 0, sd = 1)

# Finally, generate Y as influenced by both Z and X
# The true direct effect of X on Y is 0.3
# The true effect of Z on Y is 0.1
Y <- 5 + 0.3 * X + 0.1 * Z + stats::rnorm(n, mean = 0, sd = 1.5)

# Create a data frame
dag_data <- base::data.frame(X = X, Y = Y, Z = Z)

# Quick summary of the data
base::summary(dag_data)
```
...
dag_data |>
  ...
  ggplot2::ggtitle("Distributions of X, Y, and Z")
...
DiagrammeR::grViz("
  digraph DAG {
    ...
    X [label=\"X (Exposure)\", pos=\"1.0, 1.0!\", fontcolor=\"navy\"]
    Y [label=\"Y (Outcome)\", pos=\"3.0, 1.0!\", fontcolor=\"navy\"]
...
dagitty::coordinates(dag) <- base::list(
  x = base::c(X = 1, Y = 3, Z = 2),
  y = base::c(X = 1, Y = 1, Z = 3)
)

# Create nice visualization with ggdag
ggdag::ggdag(dag, edge_type = "link") + 
  ...
  ggplot2::ggtitle("DAG: X → Y with Z as confounder")
```
...
# Display model summary
base::summary(model_adjusted)

# Extract the coefficient for X
coef_adjusted <- stats::coef(model_adjusted)["X"]
```
...
comparison_df <- base::data.frame(
  Model = base::c("True Causal Effect", "Unadjusted Model (Ignores Z)", "Adjusted Model (Controls for Z)"),
  Coefficient = base::c(true_effect, coef_unadjusted, coef_adjusted),
  Error = base::c(0, coef_unadjusted - true_effect, coef_adjusted - true_effect),
  BiasPercent = base::c(0, 100 * (coef_unadjusted - true_effect) / true_effect, 
                  100 * (coef_adjusted - true_effect) / true_effect)
)

# Format for display
comparison_df$Coefficient <- base::round(comparison_df$Coefficient, 4)
comparison_df$Error <- base::round(comparison_df$Error, 4)
comparison_df$BiasPercent <- base::round(comparison_df$BiasPercent, 2)

# Display as a table
knitr::kable(comparison_df, 
      col.names = base::c("Model", "Coefficient of X on Y", "Absolute Error", "Bias (%)"),
...
model_comparison <- stats::anova(model_unadjusted, model_adjusted)

# Test if unadjusted coefficient differs from true effect
unadj_z_stat <- (coef_unadjusted - true_effect) / base::summary(model_unadjusted)$coefficients["X", "Std. Error"]
unadj_p_value <- 2 * (1 - stats::pnorm(base::abs(unadj_z_stat)))

# Test if adjusted coefficient differs from true effect
adj_z_stat <- (coef_adjusted - true_effect) / base::summary(model_adjusted)$coefficients["X", "Std. Error"]
adj_p_value <- 2 * (1 - stats::pnorm(base::abs(adj_z_stat)))
...
significance_df <- base::data.frame(
  ...
  Statistic = base::c(
    base::round(model_comparison$F[2], 3),
    base::round(unadj_z_stat, 3),
    base::round(adj_z_stat, 3)
  ),
  
  PValue = base::c(
    base::round(model_comparison$`Pr(>F)`[2], 4),
    base::round(unadj_p_value, 4),
    base::round(adj_p_value, 4)
  ),
  
  Conclusion = base::c(
    base::ifelse(model_comparison$`Pr(>F)`[2] < 0.05, 
...
knitr::kable(significance_df, 
      col.names = base::c("Comparison", "Test", "Statistic", "P-value", "Conclusion"),
...
partial_cor <- function(x, y, z, data) {
  model_x <- stats::lm(stats::as.formula(base::paste(x, "~", z)), data = data)
  model_y <- stats::lm(stats::as.formula(base::paste(y, "~", z)), data = data)
  
  residuals_x <- stats::residuals(model_x)
  residuals_y <- stats::residuals(model_y)
  
  stats::cor(residuals_x, residuals_y)
}

# Calculate simple correlation between X and Y
simple_cor_XY <- stats::cor(dag_data$X, dag_data$Y)

# Calculate partial correlation between X and Y, controlling for Z
partial_cor_XY_Z <- partial_cor("X", "Y", "Z", dag_data)

# Prepare results for display
cor_tests <- base::data.frame(
  Test = base::c("Simple correlation between X and Y", 
           "Partial correlation between X and Y, controlling for Z"),
  Correlation = base::c(simple_cor_XY, partial_cor_XY_Z)
)

# Format for display
cor_tests$Correlation <- base::round(cor_tests$Correlation, 4)

# Display as a table
knitr::kable(cor_tests, 
      col.names = base::c("Test", "Correlation"),
...
ggdag::ggdag_paths(dag, from = "X", to = "Y", shadow = TRUE) +
  ggdag::theme_dag() +
  ggplot2::ggtitle("All paths from X to Y")
```
...
ggdag::ggdag_adjustment_set(dag, exposure = "X", outcome = "Y", shadow = TRUE) +
  ggdag::theme_dag() +
  ggplot2::ggtitle("Adjustment set to estimate causal effect of X on Y")
```
...
p1 <- ggplot2::ggplot(dag_data, ggplot2::aes(x = X, y = Y)) +
  ...
  ggplot2::geom_smooth(method = "lm", formula = y ~ x, color = "firebrick") +
  ggplot2::theme_minimal() +
  ggplot2::annotate("text", x = base::min(dag_data$X) + 0.2*(base::max(dag_data$X)-base::min(dag_data$X)), 
           y = base::max(dag_data$Y) - 0.1*(base::max(dag_data$Y)-base::min(dag_data$Y)), 
           label = base::paste("Slope =", base::round(stats::coef(stats::lm(Y ~ X, dag_data))[2], 3)),
           hjust = 0) +
...
p2 <- ggplot2::ggplot(dag_data, ggplot2::aes(x = Z, y = X)) +
  ...
  ggplot2::geom_point(alpha = 0.3, color = "darkgreen") +
  ggplot2::theme_minimal() +
  ggplot2::annotate("text", x = base::min(dag_data$Z) + 0.2*(base::max(dag_data$Z)-base::min(dag_data$Z)), 
           y = base::max(dag_data$X) - 0.1*(base::max(dag_data$X)-base::min(dag_data$X)), 
           label = base::paste("Slope =", base::round(stats::coef(stats::lm(X ~ Z, dag_data))[2], 3)),
           hjust = 0) +
...
p3 <- ggplot2::ggplot(dag_data, ggplot2::aes(x = Z, y = Y)) +
  ...
  ggplot2::annotate("text", x = base::min(dag_data$Z) + 0.2*(base::max(dag_data$Z)-base::min(dag_data$Z)), 
           y = base::max(dag_data$Y) - 0.1*(base::max(dag_data$Y)-base::min(dag_data$Y)), 
           label = base::paste("Slope =", base::round(stats::coef(stats::lm(Y ~ Z, dag_data))[2], 3)),
           hjust = 0) +
  ggplot2::ggtitle("Z → Y relationship (confounder affects outcome)")

# 4. X-Y relationship (adjusted for Z) - using residuals
# Create residuals
x_resid <- stats::residuals(stats::lm(X ~ Z, data = dag_data))
y_resid <- stats::residuals(stats::lm(Y ~ Z, data = dag_data))
resid_data <- base::data.frame(x_resid = x_resid, y_resid = y_resid)
...
p4 <- ggplot2::ggplot(resid_data, ggplot2::aes(x = x_resid, y = y_resid)) +
  ...
  ggplot2::theme_minimal() +
  ggplot2::annotate("text", x = base::min(resid_data$x_resid) + 0.2*(base::max(resid_data$x_resid)-base::min(resid_data$x_resid)), 
           y = base::max(resid_data$y_resid) - 0.1*(base::max(resid_data$y_resid)-base::min(resid_data$y_resid)), 
           label = base::paste("Slope =", base::round(stats::coef(stats::lm(y_resid ~ x_resid))[2], 3)),
           hjust = 0) +
  ggplot2::xlab("X (residuals after controlling for Z)") +
  ggplot2::ylab("Y (residuals after controlling for Z)") +
  ggplot2::ggtitle("X → Y relationship after removing Z's effect")
...
dseparation_results <- base::data.frame(
  Claim = base::c(
    "X and Y are not d-separated (without conditioning)",
    "X and Y are not d-separated given Z"
  ),
  Result = base::c(
    !dagitty::dseparated(dag, "X", "Y", base::c()),
    !dagitty::dseparated(dag, "X", "Y", base::c("Z"))
  ),
  Explanation = base::c(
...
# Display as a table
knitr::kable(dseparation_results, 
      col.names = base::c("D-Separation Claim", "Confirmed?", "Explanation"),
...
simulate_unmeasured_z <- function(cor_xz, cor_zy, n = 1000) {
  ...
  z_unmeasured <- stats::rnorm(n)
  
  # Create X correlated with unmeasured Z
  x_unmeasured <- cor_xz * z_unmeasured + sqrt(1 - cor_xz^2) * stats::rnorm(n)
  ...
  y_unmeasured <- 0.3 * x_unmeasured + cor_zy * z_unmeasured + 
                  sqrt(1 - 0.3^2 - cor_zy^2 - 2*0.3*cor_xz*cor_zy) * stats::rnorm(n)
  
  # Create dataset
  base::data.frame(X = x_unmeasured, Y = y_unmeasured)
}

# Create a range of confounding strengths
z_effects <- base::seq(0, 0.5, by = 0.1)
...
for (z_effect in z_effects) {
  ...
  model <- stats::lm(Y ~ X, data = sim_data)
  
  # Store results
  results <- rbind(results, base::data.frame(
    ConfounderStrength = z_effect,
    EstimatedEffect = stats::coef(model)["X"],
    TrueEffect = 0.3,
    Bias = stats::coef(model)["X"] - 0.3,
    BiasPercent = 100 * (stats::coef(model)["X"] - 0.3) / 0.3
  ))
}

# Plot the results
ggplot2::ggplot(results, ggplot2::aes(x = ConfounderStrength, y = EstimatedEffect)) +
...
dag_data_std <- dag_data |>
  dplyr::mutate(dplyr::across(tidyselect::everything(), base::scale))
...
post <- rethinking::extract.samples(m_adjusted)

# Summarize the posterior for the causal effect of X on Y
causal_effect_summary <- base::data.frame(
  Parameter = "Effect of X on Y",
  Mean = base::mean(post$bX),
  SD = stats::sd(post$bX),
  `2.5%` = stats::quantile(post$bX, 0.025),
  `50%` = stats::median(post$bX),
  `97.5%` = stats::quantile(post$bX, 0.975)
)

# Display the causal effect estimate
knitr::kable(causal_effect_summary, 
...
ggplot2::ggplot(base::data.frame(bX = post$bX), ggplot2::aes(x = bX)) +
  ggplot2::geom_density(fill = "skyblue", alpha = 0.7) +
  ggplot2::geom_vline(xintercept = base::mean(post$bX), color = "darkblue", linetype = "dashed") +
  ggplot2::labs(
...
predict_counterfactual <- function(x_values, fixed_z = 50) {
  ...
  intercept <- stats::coef(model_adjusted)[1]
  x_coef <- stats::coef(model_adjusted)[2]
  z_coef <- stats::coef(model_adjusted)[3]
...
ggplot2::ggplot() +
  ...
  ggplot2::labs(
    ...
    subtitle = base::paste("Holding Z constant at its mean:", base::round(z_mean, 2)),
    x = "X (Exposure)",
    y = "Y (Outcome)"
  ) +
  ggplot2::theme_minimal()
...
z_values <- stats::quantile(dag_data$Z, probs = base::c(0.1, 0.25, 0.5, 0.75, 0.9))
predictions_list <- base::list()

for (z_val in z_values) {
  y_pred <- predict_counterfactual(x_range, fixed_z = z_val)
  predictions_list[[base::as.character(base::round(z_val, 1))]] <- y_pred
}

# Convert to a data frame for ggplot
predictions_df <- base::data.frame(X = x_range)
for (z_val in base::names(predictions_list)) {
  predictions_df[[paste0("Z_", z_val)]] <- predictions_list[[z_val]]
}

# Reshape for plotting
predictions_long <- predictions_df |>
  tidyr::pivot_longer(cols = tidyselect::starts_with("Z_"), 
               names_to = "Z_value", 
               values_to = "Y_pred") |>
  dplyr::mutate(Z_value = base::gsub("Z_", "", Z_value))

# Plot the counterfactual predictions for different Z values
ggplot2::ggplot() +
  # Add actual data points
  ggplot2::geom_point(data = dag_data, ggplot2::aes(x = X, y = Y), alpha = 0.1, color = "gray") +
  # Add counterfactual predictions
  ggplot2::geom_line(data = predictions_long, 
            ggplot2::aes(x = X, y = Y_pred, color = Z_value, group = Z_value),
            size = 1.0) +
  ggplot2::scale_color_viridis_d(name = "Z Value\n(Age)") +
  ggplot2::labs(
