...
digraph DAG {
  ...
  UC [label=\"Unmeasured\\nConfounder\", fontcolor=\"\", pos=\"3.0,1.5!\"]
...
is_identifiable <- dagitty::isAcyclic(dag) && 
                   base::length(dagitty::adjustmentSets(dag)) > 0
...
backdoor_paths <- base::character(0)
paths_data <- base::tryCatch({
  dagitty::paths(dag, from = "HIV", to = "Stroke")
}, error = function(e) {
  base::data.frame(paths = base::character(0), length = base::numeric(0))
})

if(base::is.data.frame(paths_data) && base::nrow(paths_data) > 0) {
  for(i in 1:base::nrow(paths_data)) {
    path_str <- paths_data$paths[i]
    path_elements <- base::strsplit(path_str, " ")[[1]]
    
    # A backdoor path has an arrow pointing into the exposure
    if(base::length(path_elements) >= 3) {
...
directed_paths <- base::tryCatch({
  dagitty::paths(dag, from = "HIV", to = "Stroke", directed = TRUE)
}, error = function(e) {
  base::data.frame(paths = base::character(0), length = base::numeric(0))
})

mediators <- base::character(0)
if(base::is.data.frame(directed_paths) && base::nrow(directed_paths) > 0) {
  mediators_list <- base::vector("list", base::nrow(directed_paths))
  
  for(i in 1:base::nrow(directed_paths)) {
    path_str <- directed_paths$paths[i]
    path_elements <- base::strsplit(path_str, " ")[[1]]
    
    # Extract variables (every other element)
    path_vars <- path_elements[base::seq(1, base::length(path_elements), by = 2)]
    
    # Variables between HIV and Stroke are mediators
    if(base::length(path_vars) > 2) {
      potential_mediators <- path_vars[-base::c(1, base::length(path_vars))]
      mediators <- base::c(mediators, potential_mediators)
    }
  }
  mediators <- base::unique(mediators)
}

# Test d-separation
d_sep_results <- base::list(
...
adjustment_effects <- base::list()
adjustment_sets_to_check <- base::list(
  "None" = base::c(),
  "Age" = base::c("Age"),
...
for(adj_name in base::names(adjustment_sets_to_check)) {
  ...
  if(base::is.data.frame(paths) && base::nrow(paths) > 0) {
    open_paths <- base::tryCatch({
      dagitty::paths(dag, from = "HIV", to = "Stroke", Z = adj_set)
    }, error = function(e) {
      base::data.frame(paths = base::character(0), length = base::numeric(0))
    })
    
    adjustment_effects[[adj_name]] <- base::list(
      "total_paths" = base::nrow(paths),
      "open_paths" = base::nrow(open_paths)
    )
  } else {
    adjustment_effects[[adj_name]] <- base::list(
      "total_paths" = 0,
...
unmeasured_impact <- base::list()
all_vars <- base::names(dagitty::coordinates(dag)$x)
...
for(var in all_vars) {
  if(var != "HIV" && var != "Stroke") {
    ...
    unmeasured_impact[[var]] <- base::list(
      "original_sets" = base::length(adj_sets_original),
      "modified_sets" = base::length(adj_sets_modified)
    )
  }
}
...
has_paths <- base::tryCatch({
  paths_data <- dagitty::paths(dag, from = "HIV", to = "Stroke")
  base::is.data.frame(paths_data) && base::nrow(paths_data) > 0
}, error = function(e) {
  FALSE
})

num_paths <- if(has_paths) base::nrow(dagitty::paths(dag, from = "HIV", to = "Stroke")) else 0

properties_df <- base::data.frame(
  Property = base::c(
    "Acyclic DAG", 
  ...
  Value = base::c(
    base::ifelse(dagitty::isAcyclic(dag), "Yes", "No"),
    base::ifelse(is_identifiable, "Yes", "No"),
    num_paths,
    base::length(backdoor_paths),
    base::ifelse("HIV" %in% dagitty::parents(dag, "Stroke"), "Yes", "No"),
    base::ifelse(base::length(mediators) > 0, base::paste(mediators, collapse=", "), "None"),
    base::length(adjustment_sets),
    base::ifelse(base::length(minimal_adjustment_sets) > 0, 
           base::paste(base::sapply(minimal_adjustment_sets, function(x) base::paste(x, collapse=", ")), collapse="; "), 
           "None")
  )
)
...
if(base::is.data.frame(paths_data) && base::nrow(paths_data) > 0) {
  paths_df <- base::data.frame(
    Path = paths_data$paths,
    Length = paths_data$length,
    IsBackdoor = base::sapply(paths_data$paths, function(p) {
      elements <- base::strsplit(p, " ")[[1]]
      if(base::length(elements) >= 3) {
        return(elements[2] == "<-")
      }
      return(FALSE)
    }),
    IsDirected = base::sapply(paths_data$paths, function(p) {
      elements <- base::strsplit(p, " ")[[1]]
      all_forward <- TRUE
      for(i in base::seq(2, base::length(elements), by=2)) {
        if(elements[i] != "->") {
          all_forward <- FALSE
          break
        }
  ...
  base::cat("No paths found between HIV and Stroke")
}
...
if(base::length(independencies) > 0) {
  # Create a clean format for the independencies
  ind_statements <- base::character(base::length(independencies))
  
  for(i in 1:base::length(independencies)) {
    ind <- independencies[[i]]
    if(base::length(ind) >= 2) {
      statement <- base::paste0(ind[1], " _||_ ", ind[2])
      
      if(base::length(ind) > 2 && base::length(ind[[3]]) > 0) {
        statement <- base::paste0(statement, " | ", base::paste(ind[[3]], collapse=", "))
      } else {
        statement <- base::paste0(statement, " (unconditionally)")
      }
  ...
  independencies_df <- base::data.frame(
  ...
  base::cat("No conditional independencies implied by this DAG")
}
...
relations_df <- base::data.frame(
  Variable = base::c("HIV", "Stroke"),
  Ancestors = base::c(
    base::paste(dplyr::setdiff(hiv_ancestors, "HIV"), collapse=", "),
    base::paste(dplyr::setdiff(stroke_ancestors, "Stroke"), collapse=", ")
  ),
  Descendants = base::c(
    base::paste(dplyr::setdiff(hiv_descendants, "HIV"), collapse=", "),
    base::paste(dplyr::setdiff(stroke_descendants, "Stroke"), collapse=", ")
  )
)
...
adj_effects_df <- base::data.frame(
  AdjustmentSet = base::names(adjustment_effects),
  TotalPaths = base::sapply(adjustment_effects, function(x) x$total_paths),
  OpenPaths = base::sapply(adjustment_effects, function(x) x$open_paths),
  ClosedPaths = base::sapply(adjustment_effects, function(x) x$total_paths - x$open_paths)
)
...
unmeasured_df <- base::data.frame(
  Variable = base::names(unmeasured_impact),
  OriginalAdjustmentSets = base::sapply(unmeasured_impact, function(x) x$original_sets),
  AdjustmentSetsIfUnmeasured = base::sapply(unmeasured_impact, function(x) x$modified_sets),
  IdentifiableIfUnmeasured = base::sapply(unmeasured_impact, function(x) x$modified_sets > 0)
)
...
d_sep_df <- base::data.frame(
  ...
  Result = base::c(
    base::ifelse(d_sep_results$Smoke_stroke, "Yes (d-separated)", "No (d-connected)"),
    base::ifelse(d_sep_results$age_uc, "Yes (d-separated)", "No (d-connected)"),
    base::ifelse(d_sep_results$hiv_uc, "Yes (d-separated)", "No (d-connected)")
  )
)
...
ggdag::ggdag_adjustment_set(dag_tidy) +
  ggdag::theme_dag() +
  ggplot2::labs(title = "Adjustment Sets for HIV → Stroke")

# Paths visualization
ggdag::ggdag_paths(dag_tidy) +
  ggdag::theme_dag() +
  ggplot2::labs(title = "All Paths between HIV and Stroke")
