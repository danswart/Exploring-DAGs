...
for(var in variables_to_plot) {
  p <- ggplot2::ggplot(dag_data, ggplot2::aes(x = "X_group", y = var, fill = "X_group")) +
...
dag_data_std <- dag_data  |>
  dplyr::mutate(dplyr::across(tidyselect::where(base::is.numeric), base::scale))  |>
  base::data.frame()
...
forest_data <- comparison_df  |>
  ...
  dplyr::mutate(
    Model = base::factor(Model, levels = base::rev(base::c(
      "Unadjusted (Correct for RCT)",
      "Adjusted for Z (Prognostic)",
      "Adjusted for C (Prognostic)", 
      "Adjusted for B (Prognostic)",
...
simpson_analysis <- simpson_analysis  |>
  dplyr::mutate(dplyr::across(tidyselect::where(base::is.numeric), ~ base::round(., 3)))
...
p4 <- ggplot2::ggplot(slope_comparison, ggplot2::aes(x = reorder(Group, Slope), y = Slope, fill = Type)) +
...
magnitude_analysis <- base::data.frame(
  ...
  Direction_Reversal = base::c(
    base::ifelse(base::any(sign(stratified_results$slope_XY) != sign(overall_slope)), "Yes", "No"),
    base::ifelse(base::any(sign(binary_Z$slope_XY) != sign(overall_slope)), "Yes", "No"),
    base::ifelse(base::any(sign(binary_C$slope_XY) != sign(overall_slope)), "Yes", "No"),
    base::ifelse(base::any(sign(binary_B$slope_XY) != sign(overall_slope)), "Yes", "No")
  )
)
