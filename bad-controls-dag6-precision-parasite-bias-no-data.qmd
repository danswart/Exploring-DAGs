---
title: "Bad Controls: Precision Parasite Bias"
author: "Dan Swart"
format: 
  html:
    toc: true
...
ggdag_status(precision_parasite_bias_dag6) + 
  theme_dag(base_size = 18) +
  labs(title = "Bad Controls: Precision Parasite Bias")

```

## Executive Summary: Precision Parasite Bias as a Bad Control

::: {.callout-note collapse="true" title="<span style='font-size: 20px;'>What is Precision Parasite Bias?</span> <span style='color: darkblue; font-size: 22px;'>(click to open and close)</span>"}


Precision parasite bias occurs when we adjust for a variable Z that is a cause of the exposure X but not a confounder. In this DAG structure, Z affects X but has no direct path to Y, and controlling for Z can reduce precision in our estimate of the causal effect of X on Y without reducing bias.

#### Why is it a "Bad Control"?

Controlling for Z in this structure is potentially harmful because:

1. **It reduces statistical precision**: By conditioning on Z, we're removing some of the variation in X that could be used to estimate the effect of X on Y, leading to less precise estimates.

2. **It doesn't reduce confounding**: Since Z only affects X and not Y through any other path, it's not a confounder and doesn't need to be controlled for to identify the causal effect.

3. **It can amplify the impact of measurement error**: If X is measured with error, conditioning on Z can increase the bias due to measurement error in the effect estimate.
...
- The patient's weight (Z) does not directly affect recovery outcome (Y) nor is it related to any other variable affecting Y.

- If the researcher controls for patient weight (Z), they unnecessarily reduce the variation in medication dosage (X), leading to less precise estimates without reducing any bias.

#### How to Avoid Precision Parasite Bias

1. **Identify true confounders**: Focus on controlling for variables that affect both the treatment and outcome.

2. **Avoid controlling for causes of exposure only**: Don't adjust for variables that only affect the exposure and have no other path to the outcome.

3. **Preserve statistical power**: Only control for variables necessary for causal identification to maintain precision in estimates.

4. **Use directed acyclic graphs (DAGs)**: Visualize the causal structure to identify appropriate control variables.

Precision parasite bias demonstrates that controlling for more variables is not always better - unnecessary adjustment can reduce precision without improving bias.
...
# Run the analysis
dag_results <- display_dag_analysis(precision_parasite_bias_dag6)
...
properties_df <- data.frame(
  ...
  Value = c(
    ifelse(dagitty::isAcyclic(precision_parasite_bias_dag6), "Yes", "No"),
    ...
    ifelse("X" %in% dagitty::parents(precision_parasite_bias_dag6, "Y"), "Yes", "No"),
...
dag_plot <- ggdag(precision_parasite_bias_dag6) + 
  theme_dag() +
  label("DAG: Precision Parasite Bias")
...
### 4.1 Key Insights about this Precision Parasite Bias DAG Structure

This DAG represents a causal network with a precision parasite bias structure, examining the relationship between X and Y where Z affects X:
...
2. **Precision Parasite Variable (Z)**
   ...
   - This creates a precision parasite bias structure

3. **Potential Pitfalls**
   - Conditioning on Z reduces the variability in X
   - This makes it harder to precisely estimate the effect of X on Y
   - The statistical precision of our estimate would be reduced if we control for Z

4. **Why This Happens**
   - When we condition on Z, we're removing some of the variation in X that could be used to estimate its effect on Y
   - This reduces the effective sample size or information available for estimation
   - The standard errors of our estimates tend to increase without any reduction in bias
...
To identify the causal effect of X on Y:
  ...
  - Do not adjust for Z as it's not a confounder
  - Z could potentially be used as an instrumental variable if there are unmeasured confounders
  - The most efficient strategy is to make no adjustment
  - Adjusting for Z will lead to precision parasite bias by unnecessarily reducing the precision of our estimate
...
**Instrumental Variable**: A variable that affects the exposure but has no direct effect on the outcome except through the exposure. In this DAG, Z may function as an instrumental variable for the effect of X on Y.

**Precision Parasite Bias**: A type of bias that occurs when we condition on a variable Z that affects the exposure X but is not a confounder (does not affect Y through any path except possibly through X). This conditioning reduces the precision of our estimates without reducing confounding bias.
...
- Z is an ancestor of X but not Y in this DAG
...
Lists potential instrumental variables - variables that affect the exposure but have no direct effect on the outcome except through the exposure. In this DAG, Z may be identified as an instrumental variable for the effect of X on Y, which could be useful if there were unmeasured confounders.
...
2. **Avoid conditioning on precision parasites**: In this DAG, Z is a precision parasite, so conditioning on Z will reduce the precision of the estimate of X on Y without reducing bias.

3. **Validate your DAG**: Use the implied conditional independencies to test your causal assumptions against observed data.

4. **Consider using Z as an instrumental variable**: If there were unmeasured confounders between X and Y, Z could potentially be used as an instrumental variable to estimate the causal effect of X on Y.

5. **Look for precision parasite structure**: In this particular DAG, the structure Z → X → Y indicates that Z is a precision parasite. Adjusting for Z would induce a precision loss in the estimate of X on Y.
