...
p1 <- ggplot2::ggplot(dag_data, ggplot2::aes(x = X, y = Y)) +
  ...
  ggplot2::geom_smooth(method = "lm", formula = y ~ x, color = "darkred") +
  ggplot2::theme_minimal() +
  ggplot2::annotate("text", x = min(dag_data$X) + 0.2*(max(dag_data$X)-min(dag_data$X)), 
...
p2 <- ggplot2::ggplot(dag_data, ggplot2::aes(x = Z, y = X)) +
  ...
  ggplot2::geom_point(alpha = 0.3, color = "darkgreen") +
  ggplot2::theme_minimal() +
  ggplot2::annotate("text", x = min(dag_data$Z) + 0.2*(max(dag_data$Z)-min(dag_data$Z)), 
...
p4 <- ggplot2::ggplot(resid_data, ggplot2::aes(x = x_resid, y = y_resid)) +
  ...
  ggplot2::theme_minimal() +
  ggplot2::annotate("text", x = min(resid_data$x_resid) + 0.2*(max(resid_data$x_resid)-min(resid_data$x_resid)), 
...
dseparation_results <- data.frame(
  ...
  Result = c(
    !dagitty::dseparated(dag, "X", "Y", c()),
    !dagitty::dseparated(dag, "X", "Y", c("Z"))
  ),
  Explanation = c(
...
ggplot2::ggplot(data.frame(bX = post$bX), ggplot2::aes(x = bX)) +
  ggplot2::geom_density(fill = "skyblue", alpha = 0.7) +
  ggplot2::geom_vline(xintercept = mean(post$bX), color = "darkblue", linetype = "dashed") +
  ggplot2::labs(
...
predictions_long <- predictions_df |>
  tidyr::pivot_longer(cols = dplyr::starts_with("Z_"), 
               names_to = "Z_value", 
               values_to = "Y_pred") |>
  dplyr::mutate(Z_value = gsub("Z_", "", Z_value))
...
p1 <- ggplot2::ggplot(dag_data, ggplot2::aes(x = X, y = Y)) +
  ...
  ggplot2::geom_smooth(method = "lm", formula = y ~ x, color = "red", size = 1.5, 
...
p2 <- ggplot2::ggplot(dag_data, ggplot2::aes(x = X, y = Y)) +
  ...
  ggplot2::scale_color_manual(values = c("High Z" = "darkblue", "Low Z" = "darkgreen"),
                     name = "Z Group") +
  ggplot2::labs(
...
p3 <- ggplot2::ggplot(slope_comparison, ggplot2::aes(x = reorder(Group, Slope), y = Slope, fill = Type)) +
  ggplot2::geom_col(alpha = 0.8) +
  ggplot2::geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
  ggplot2::geom_hline(yintercept = 0.3, linetype = "dotted", color = "red", alpha = 0.7) +
  ggplot2::scale_fill_manual(values = c("Overall" = "red", "Quartile" = "steelblue", "Binary" = "darkgreen")) +
  ggplot2::labs(
...
group_analysis_complete <- dplyr::bind_rows(group_size_analysis, summary_row)
