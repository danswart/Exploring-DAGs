...
# All libraries are now called with namespace syntax (package::function)
...
ggdag::ggdag(dag) + 
  ggdag::theme_dag() +
  ggplot2::labs(title = "DAG: X -> Y with Z as confounder")
```
...
display_dag_analysis <- function(dag) {
  ...
  # 4. Identify all paths between exposure and outcome
...
# Set coordinates for visualization in dagitty format (1 = TOP, 3 = BOTTOM)
dagitty::coordinates(dag) <- list(
  x = c(X = 1.0, Y = 3.0, Z = 2.0),
  y = c(X = 1.0, Y = 1.0, Z = 3.0)
)
