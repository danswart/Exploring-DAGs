---
title: "DAG Analysis - Bad Controls: Selection Bias"
subtitle: ""
description: ""
author: 
...
grViz("
  digraph DAG {
    ...
    Z [label=\"Z\", pos=\"2.0, 2.0!\", fontcolor=\"black\"]
...
selection_bias_dag3 <- ggdag::dagify(
  ...
  labels = c(X = "X", 
             Y = "Y", 
             Z = "Z"),
  coords = list(x = c(X = 1, Y = 3, Z = 2.0),  
                y = c(X = 1, Y = 1, Z = 2.0))
)

# Create a nice visualization of the DAG
ggdag_status(selection_bias_dag3) + 
  theme_dag(base_size = 18) +
  labs(title = "Bad Controls: Selection Bias")

```

## Executive Summary: Selection Bias as a Bad Control

::: {.callout-note collapse="true" title="<span style='font-size: 20px;'>What is Selection  Bias?</span> <span style='color: darkblue; font-size: 22px;'>(click to open and close)</span>"}


Selection bias occurs when we control for a variable Z that is a common effect (collider) of both our exposure X and outcome Y, or their causes. In this DAG, Z is directly influenced by both X and Y.

#### Why is it a "Bad Control"?

Controlling for Z is harmful because:

1. **It induces a spurious association**: Conditioning on a collider like Z creates a non-causal association between X and Y, even if they were independent in the underlying population.

2. **It can reverse the true causal effect**: In some cases, controlling for a collider can make a positive causal effect appear negative, or vice versa.

3. **It's commonly misunderstood**: Researchers often control for all measured variables without considering the causal structure, leading to selection bias.

#### Real-World Example

A researcher is studying the effect of a new teaching method (X) on student test scores (Y):

- Both the teaching method (X) and student test scores (Y) affect whether students are selected for a special program (Z) in the first place.

- If the researcher controls for program participation (Z), they will induce a spurious association between teaching method (X) and test scores (Y)

- This could lead to biased estimates of the teaching method's effectiveness

#### How to Avoid Selection Bias

1. **Identify colliders in your causal graph**: Look for variables that are common effects of other variables

2. **Avoid controlling for colliders**: Unless there's a compelling reason, do not adjust for variables that are affected by both exposure and outcome

3. **Consider the temporal ordering**: Variables occurring after both the exposure and outcome may be colliders
4. **Prioritize pre-treatment variables**: When selecting controls, focus on variables that precede the exposure in time

Selection bias illustrates why understanding the causal structure is crucial for selecting appropriate control variables.
...
# Run the analysis
dag_results <- display_dag_analysis(selection_bias_dag3)
...
properties_df <- data.frame(
  ...
  Value = c(
    ifelse(dagitty::isAcyclic(selection_bias_dag3), "Yes", "No"),
    ...
    ifelse("X" %in% dagitty::parents(selection_bias_dag3, "Y"), "Yes", "No"),
...
dag_plot <- ggdag(selection_bias_dag3) + 
  theme_dag() +
  label("DAG: Selection Bias causal structure")
...
### 4.1 Key Insights about this Selection Bias DAG Structure

This DAG represents a causal network known as a Selection Bias structure, examining the relationship between X and Y:

1. **Direct Causal Effect (X → Y)**
   - X directly affects Y
   - This represents the causal effect we're interested in measuring

2. **Selection Bias Structure**
   - Z is influenced by both X and Y
   - Z is a collider on the path X → Z ← Y
   - This creates a selection bias structure

3. **Potential Pitfalls**
   - Conditioning on Z (the collider) opens a non-causal path between X and Y
   - This would induce a spurious association between X and Y
   - The estimated effect of X on Y would be biased

### 4.2 Proper Identification Strategy

To identify the causal effect of X on Y:
  - No adjustment is necessary in this simple DAG
  - Do not adjust for Z as it's a collider
  - The most efficient strategy is to make no adjustment
  - Adjusting for Z will lead to selection bias by creating a spurious association
...
7. **Recognize Selection Bias structure**: In this particular DAG, adjusting for Z (the collider) would induce a spurious association between X and Y - a classic selection bias problem.
