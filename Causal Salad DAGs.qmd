...
### Same diagram created as a dagitty DAG and analyzed with dagitty functions
...
# Set graphics parameters FIRST
par(mar = c(5, 1, 1, 1), 
    lwd = 3)  # This affects line width

# Plot ONCE with all settings
plot(dag, 
     cex = 1.5,            # Overall scaling
     cex.lab = 1.0,        # Label size
     cex.axis = 1.2,       # Axis text size
     lwd = 4.5,            # Line thickness
     arr.lwd = 5,          # Arrow line thickness
     arr.width = 1.00,     # Arrow head width (proportion)
     arr.type = "curved"   # Arrow style
...
ggdag::ggdag_status(causal_salad_dag1) +
  ggplot2::ggtitle("Causal Salad DAG #1 - the Fork Z") +
...
### Causal Saled DAG #2 - Fork + Collider
...
ggdag::ggdag_status(causal_salad_dag2) +
  ggplot2::ggtitle("Causal Salad DAG #2-Fork Z and Collider C") +
  ggplot2::guides(fill = "none", color = "none") # Disable the legend
...
# Now, graphically
ggdag::ggdag_adjustment_set(causal_salad_dag2,
                            shadow = TRUE)
...
### Causal Saled DAG #3 - Multiple Forks and Colliders
...
ggdag::ggdag_status(causal_salad_dag3) +
  ...
  ggplot2::guides(fill = "none", color = "none") + # Disable the legend
  ggdag::theme_dag(base_size = 18)  # This removes axes and grid
...
### Causal Salad DAG #4 - No Backdoor Paths
...
ggdag::ggdag_status(causal_salad_dag4) +
  ggplot2::ggtitle("Causal Salad DAG #4 - No Backdoor Paths") +
  ggplot2::guides(fill = "none", color = "none") + # Disable the legend
  ggdag::theme_dag(base_size = 18)  # This removes axes and grid
...
### Causal Salad DAG #5 - HIV and Stroke
...
causal_salad_dag5 <- ggdag::dagify(
  ...
  labels = c(X = "HIV", 
             ...
             A = "Smokimg", 
             B = "Age"),
  coords = list(x = c(X = 2.00, Y = 3.00, A = 1.00, B = 1.00),
                y = c(X = 2.00, Y = 2.00, A = 4.00, B = 1.00)
                )
)

ggdag::ggdag_status(causal_salad_dag5, use_labels = "label", text = FALSE) +
  ggplot2::ggtitle("Causal Salad DAG #5 - HIV and Stroke") +
  ggplot2::guides(fill = "none", color = "none") + # Disable the legend
  ggdag::theme_dag(base_size = 18)  # This removes axes and grid
...
### Causal Salad DAG #6 - HIV and Stroke + Unmeasured Confounder
...
ggdag::ggdag_status(causal_salad_dag6, use_labels = "label", text = FALSE) +
  ggplot2::ggtitle("Causal Salad DAG #6 - HIV and Stroke + Unmeasured Confounder") +
  ggplot2::guides(fill = "none", color = "none") + # Disable the legend
  ggdag::theme_dag()
...
ggdag::ggdag_paths(causal_salad_dag6, use_labels = "label") +
  ggplot2::ggtitle("Causal Salad DAG #6 - HIV and Stroke with Unmeasured Confounder") +
  ggplot2::guides(fill = "none", color = "none") + # Disable the legend
  ggdag::theme_dag(base_size = 18)  # This removes axes and grid
...
# Build Selection-Bias DAG with ggdag and dagitty
...
### Bad Controls - Modified Selection Bias
...
# Build Modified Selection-Bias DAG with ggdag mad dagitty
...
### Bad Controls - Case-Control Bias
...
### Bad Controls - Precision Parasite
