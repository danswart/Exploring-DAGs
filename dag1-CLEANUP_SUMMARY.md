...
- **Line 608**: Fixed incorrect `partial_stats::cor()` to `partial_cor()` (user-defined function, not a package function)
- **Lines 863-868**: Removed namespace prefixes from distribution functions inside `rethinking::quap()` model formula. The `quap()` function uses non-standard evaluation and requires bare function names like `dnorm()` and `dexp()` without `stats::` prefix inside `alist()`

## Important Notes

**Special cases where namespace prefixes cannot be used:**
1. Distribution functions inside `rethinking::quap()` model formulas (e.g., `dnorm()`, `dexp()`)
2. User-defined functions (e.g., `partial_cor()`, `simulate_unmeasured_z()`, `predict_counterfactual()`)

These are rare exceptions - the rethinking package uses non-standard evaluation that parses formulas as text, which breaks with namespace prefixes.
