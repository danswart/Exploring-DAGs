...
# library(DiagrammeR) # Graph/Network Visualization # For complete control of the layout
...
ggdag::ggdag(causal_salad_dag6) + 
  ggdag::theme_dag() +
  ggdag::label("DAG: HIV-Stroke causal structure with unmeasured confounding")
...
ancestors_descendants_df <- data.frame(
  ...
  Ancestors = c(
    paste(dplyr::setdiff(dag_results$X_ancestors, ""), collapse=", "),
    paste(dplyr::setdiff(dag_results$Y_ancestors, ""), collapse=", "),
    paste(dplyr::setdiff(dag_results$A_ancestors, ""), collapse=", "),
    paste(dplyr::setdiff(dag_results$B_ancestors, ""), collapse=", "),
    paste(dplyr::setdiff(dag_results$U_ancestors, ""), collapse=", ")
  ),
  Descendants = c(
    paste(dplyr::setdiff(dag_results$X_descendants, ""), collapse=", "),
    paste(dplyr::setdiff(dag_results$Y_descendants, ""), collapse=", "),
    paste(dplyr::setdiff(dag_results$A_descendants, ""), collapse=", "),
    paste(dplyr::setdiff(dag_results$B_descendants, ""), collapse=", "),
    paste(dplyr::setdiff(dag_results$U_descendants, ""), collapse=", ")
  )
)
...
# Create a nice visualization of the DAG
dag_plot <- ggdag::ggdag(causal_salad_dag6) + 
  ggdag::theme_dag() +
  ggdag::label("DAG: HIV-Stroke causal structure with unmeasured confounding")
