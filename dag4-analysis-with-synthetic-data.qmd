...
dag_data %>%
  ...
  ggplot2::ggplot(gghighlight::aes(x = Value)) +
...
# Z vs Y scatterplot
ggplot2::ggplot(dag_data, gghighlight::aes(x = Z, y = Y)) +
...
# Display model summary
summary_adj_precision <- lavaan::summary(model_adjusted_precision)
...
comparison_df <- data.frame(
  ...
  StandardError = c(
    NA,
    lavaan::summary(models[["Unadjusted (Correct for Experimental Design)"]])$coefficients["X", "Std. Error"],
    lavaan::summary(models[["Adjusted for Z"]])$coefficients["X", "Std. Error"],
    lavaan::summary(models[["Adjusted for C"]])$coefficients["X", "Std. Error"],
    lavaan::summary(models[["Adjusted for B"]])$coefficients["X", "Std. Error"],
    lavaan::summary(models[["Adjusted for Z, C"]])$coefficients["X", "Std. Error"],
    lavaan::summary(models[["All variables"]])$coefficients["X", "Std. Error"]
  )
)
...
comparison_df$R2 <- c(
  NA,
  lavaan::summary(models[["Unadjusted (Correct for Experimental Design)"]])$r.squared,
  lavaan::summary(models[["Adjusted for Z"]])$r.squared,
  lavaan::summary(models[["Adjusted for C"]])$r.squared,
  lavaan::summary(models[["Adjusted for B"]])$r.squared,
  lavaan::summary(models[["Adjusted for Z, C"]])$r.squared,
  lavaan::summary(models[["All variables"]])$r.squared
)
...
p2 <- ggplot2::ggplot(dag_data, gghighlight::aes(x = X, y = Y, color = C_strata)) +
  ggplot2::geom_point(alpha = 0.5) +
  ggplot2::geom_smooth(method = "lm", se = TRUE) +
...
for(var in variables_to_plot) {
  p <- ggplot2::ggplot(dag_data, ggplot2::aes_string(x = "X_group", y = var, fill = "X_group")) +
    ...
    ggplot2::theme_minimal() +
    ggplot2::labs(title = paste("Distribution of", var, "by Exposure Group"),
...
calculate_power <- function(effect_size, n, alpha = 0.05) {
  ...
  residual_se <- lavaan::summary(model_unadjusted)$sigma
...
# Plot forest plot
ggplot2::ggplot(forest_data, ggplot2::aes(x = Coefficient, y = Model,
...
step1_significant <- lavaan::summary(step1_model)$coefficients["X", "Pr(>|t|)"] < 0.05

# Step 2: X should predict the mediator Z
step2_model <- stats::lm(Z ~ X, data = dag_data)  
step2_significant <- lavaan::summary(step2_model)$coefficients["X", "Pr(>|t|)"] < 0.05

# Step 3: Z should predict Y when controlling for X
step3_model <- stats::lm(Y ~ X + Z, data = dag_data)
step3_significant <- lavaan::summary(step3_model)$coefficients["Z", "Pr(>|t|)"] < 0.05
...
mediation_results <- data.frame(
  ...
  P_Value = c(
    lavaan::summary(step1_model)$coefficients["X", "Pr(>|t|)"],
    lavaan::summary(step2_model)$coefficients["X", "Pr(>|t|)"],
    lavaan::summary(step3_model)$coefficients["Z", "Pr(>|t|)"],
    lavaan::summary(step3_model)$coefficients["X", "Pr(>|t|)"]
  ),
